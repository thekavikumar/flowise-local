...
const NodeInputHandler = ({
  ...
  const [showFormatPromptValuesDialog, setShowFormatPromptValuesDialog] =
    useState(false);
  const [formatPromptValuesDialogProps, setFormatPromptValuesDialogProps] =
    useState({});
  const [showInput, setShowInput] = useState(
    data?.inputs?.sideEffects ? true : false
  ); // For sideEffects
                ...
                onChange={(newValue) => {
                  const parsedValue =
                    inputParam.type === "number"
                      ? parseFloat(newValue)
                      : newValue;
                  return (data.inputs[inputParam.name] = parsedValue);
                }}
                value={
                  typeof data.inputs[inputParam.name] === "object"
                    ? JSON.stringify(data.inputs[inputParam.name])
              ...
              inputParam.label === "SideEffects" && (
                <>
                  ...
                  >
                    ...
                    <AccordionDetails onTransitionEnd={() => {}}>
                      <Input
                        key={data.inputs[inputParam.name]}
                        disabled={false}
                        inputParam={inputParam}
                        ...
                        showDialog={showExpandDialog}
                        onExpandDialogClicked={onExpandDialogClicked}
                        dialogProps={expandDialogProps}
                        onDialogCancel={() => setShowExpandDialog(false)}
                        onDialogConfirm={(newValue, inputParamName) =>
                          onExpandDialogSave(newValue, inputParamName)
                  ...
                  {(() => {
                    try {
                      if (data.inputs[inputParam.name]) {
                        return (
                          <Editor
                            value={JSON.parse(data.inputs[inputParam.name])}
                            onChange={(newValue) =>
                              (data.inputs[inputParam.name] =
                      ...
                      return <div>Error: Invalid JSON format</div>;
                    }
                  })()}
                </div>
              )}
            ...
            {inputParam.type === "asyncOptions" && (
              <>
                ...
                >
                  ...
                  />
                  {EDITABLE_TOOLS.includes(inputParam.name) &&
                    data.inputs[inputParam.name] && (
                      <IconButton
                        title="Edit"
                        color="primary"
