...
const CanvasNode = ({ data }) => {
  ...
  const [showModal, setModalDialog] = useState(false);
  const [labelName, setLabelName] = useState("");
  const [dialogProps, setDialogProps] = useState({});

  // console.log('data:', data)

  const addSwitchOutputs = () => {
    let newOutput = {
      "id": `switch-output-${labelName}-xMessage`,
      "name": `${labelName}`,
      "label": `${labelName}`,
      "type": "xMessage",
    };
    data.outputAnchors.push(newOutput);
  };
    ...
    <>
      ...
      >
        <Box>
          ...
          <Divider />
          {data.category === "SwitchCaseTransformer" && (
            <Button
              sx={{ borderRadius: 25, width: "90%", mb: 2, ml: 2 }}
              variant="outlined"
      ...
      {showModal && (
        <Dialog open={showModal} onClose={() => setModalDialog(false)}>
          <div
            style={{
              display: "flex",
