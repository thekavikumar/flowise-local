...
const CanvasHeader = ({
  chatflow,
  handleSaveFlow,
  handleSaveCompilerData,
  handleDeleteFlow,
  handleLoadFlow,
  handleSaveFlowiseData,
  handleDownloadFlowiseData,
  handleUndo,
  handleRedo,
}) => {
  ...
  const onSettingsItemClick = (setting) => {
    ...
    if (setting === "deleteChatflow") {
      ...
      try {
        // chatflow.flowdata is string when loaded via a file but object when loaded via api
        const flowData =
          typeof chatflow.flowData === "string"
            ? JSON.parse(chatflow.flowData)
            : chatflow.flowData;
        console.log("exported flowdata:", generateExportFlowData(flowData));
        let dataStr = JSON.stringify(generateExportFlowData(flowData));
        let dataUri =
          "data:application/json;charset=utf-8," + encodeURIComponent(dataStr);
        let exportFileDefaultName = `${chatflow.name}.json`;
  ...
  const onAPIDialogClick = () => {
    ...
    setAPIDialoghandleSaveFlowOpen(true);
  };
  const onDownloadFlowiseClick = () => {
    console.log(chatflow);
    if (chatflow.id) {
      handleDownloadFlowiseData(flowName);
    } else setFlowDialogOpen(true);
  };
  const onSaveFLowiseClick = () => {
    if (chatflow.id) {
      handleSaveFlowiseData(flowName);
    } else setFlowDialogOpen(true);
  };
  const onSaveChatflowClick = () => {
    if (chatflow.id) {
      handleSaveCompilerData(flowName);
    } else setFlowDialogOpen(true);
  };
  ...
  return (
    <>
      ...
      <Box>
        ...
        <ButtonBase title="Undo" sx={{ borderRadius: "50%", mr: 2 }}>
          <Avatar
            variant="rounded"
            sx={{
              ...theme.typography.commonAvatar,
        ...
        <ButtonBase title="Redo" sx={{ borderRadius: "50%", mr: 2 }}>
          <Avatar
            variant="rounded"
            sx={{
              ...theme.typography.commonAvatar,
        ...
        >
          <Avatar
            ...
            color="inherit"
            onClick={onDownloadFlowiseClick}
          >
            <IconDownload stroke={1.5} size="1.3rem" />
          </Avatar>
        </ButtonBase>
        <ButtonBase
          title="Save Flowise Draft"
          sx={{ borderRadius: "50%", mr: 2 }}
        >
          <Avatar
            variant="rounded"
            ...
            color="inherit"
            onClick={onSaveFLowiseClick}
          >
            <IconDeviceFloppy stroke={1.5} size="1.3rem" />
          </Avatar>
        </ButtonBase>
        <ButtonBase title="Upload Logic" sx={{ borderRadius: "50%", mr: 2 }}>
          <Avatar
            variant="rounded"
            sx={{
              ...theme.typography.commonAvatar,
...
CanvasHeader.propTypes = {
  ...
  handleSaveCompilerData: PropTypes.func,
