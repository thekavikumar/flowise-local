...
const ToolDialog = ({
  ...
  const addNewRow = () => {
    setTimeout(() => {
      setToolSchema((prevRows) => {
        ...
        const lastRowId = allRows.length
          ? allRows[allRows.length - 1].id + 1
          : 1;
        allRows.push({
          id: lastRowId,
          property: "",
  ...
  useEffect(() => {
    if (getSpecificToolApi.data) {
      ...
      if (getSpecificToolApi.data.func)
        setToolFunc(getSpecificToolApi.data.func);
      else setToolFunc("");
    }
        ...
        <Box sx={{ p: 2 }}>
          <Stack sx={{ position: "relative" }} direction="row">
            <Typography variant="overline">
              ...
              <TooltipWithParser
                style={{ marginLeft: 10 }}
                title={"What should be the output response in JSON format?"}
              />
            </Typography>
          </Stack>
          <Grid
